...
func New(ctx context.Context, configFilePath, vaultName, keyName, keyVersion1, keyVersion2 string, proxyMode bool, proxyAddress string, proxyPort int) (*KeyManagementServiceServer, error) {
	cfg, err := config.GetAzureConfig(configFilePath)
	if err != nil {
		return nil, err
	}
	kvClient, err := newKeyVaultClient(cfg, vaultName, keyName, keyVersion1, keyVersion2, proxyMode, proxyAddress, proxyPort)
	if err != nil {
		return nil, err
	}
